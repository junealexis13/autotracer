...
def extract_shorelines(metadata, settings):
    ...
    for satname in metadata.keys():
        ...
        # get images
        filepath = tools.get_filepath(settings['inputs'],satname)
        ...
        for i in range(len(filenames)):
            ...
            st.toast('\r%s:   %d%%' % (satname,int(((i+1)/len(filenames))*100)))

            # get image filename
            fn = tools.get_filenames(filenames[i],filepath, satname)
            # preprocess image (cloud mask + pansharpening/downsampling)
            im_ms, georef, cloud_mask, im_extra, im_QA, im_nodata = tools.preprocess_single(fn, satname, 
            ...
            # otherwise use find_wl_contours1 (traditional)
                ...
                try: # use try/except structure for long runs
                    if sum(im_labels[im_ref_buffer,0]) < 50: # minimum number of sand pixels
                        ...
                        im_mndwi = tools.nd_index(im_ms[:,:,4], im_ms[:,:,1], cloud_mask)
    ...
    output = tools.merge_output(output)
...
def calculate_features(im_ms, cloud_mask, im_bool):
    ...
    for k in range(im_ms.shape[2]):
        im_std =  tools.image_std(im_ms[:,:,k], 1)
    ...
    im_std = tools.image_std(im_NIRG, 1)
    features = np.append(features, np.expand_dims(im_std[im_bool],axis=1), axis=-1)
    im_std = tools.image_std(im_SWIRG, 1)
    features = np.append(features, np.expand_dims(im_std[im_bool],axis=1), axis=-1)
    im_std = tools.image_std(im_NIRR, 1)
    features = np.append(features, np.expand_dims(im_std[im_bool],axis=1), axis=-1)
    im_std = tools.image_std(im_SWIRNIR, 1)
    features = np.append(features, np.expand_dims(im_std[im_bool],axis=1), axis=-1)
    im_std = tools.image_std(im_BR, 1)
    features = np.append(features, np.expand_dims(im_std[im_bool],axis=1), axis=-1)
...
def create_shoreline_buffer(im_shape, georef, image_epsg, pixel_size, settings):
    ...
    if 'reference_shoreline' in settings.keys():
        ...
        ref_sl_conv = tools.convert_epsg(ref_sl, settings['output_epsg'],image_epsg)
        ref_sl_pix = tools.convert_world2pix(ref_sl_conv, georef)
...
def adjust_detection(im_ms, cloud_mask, im_nodata, im_labels, im_ref_buffer, image_epsg, georef,
    ...
    im_RGB = tools.rescale_image_intensity(im_ms[:,:,[2,1,0]], cloud_mask, 99.9)
    ...
    if len(shoreline) > 0:
        sl_pix = tools.convert_world2pix(tools.convert_epsg(shoreline,
...
def find_wl_contours2(im_ms, im_labels, cloud_mask, im_ref_buffer):
    ...
    im_mwi = tools.nd_index(im_ms[:,:,4], im_ms[:,:,1], cloud_mask)
    # calculate Normalized Difference Modified Water Index (NIR - G)
    im_wi = tools.nd_index(im_ms[:,:,3], im_ms[:,:,1], cloud_mask)
...
def process_shoreline(contours, cloud_mask, im_nodata, georef, image_epsg, settings):
    ...
    contours_world = tools.convert_pix2world(contours, georef)
    # convert world coordinates to desired spatial reference system
    contours_epsg = tools.convert_epsg(contours_world, image_epsg, settings['output_epsg'])
    ...
    if sum(sum(cloud_mask)) > 0:
        ...
        coords_cloud = tools.convert_epsg(tools.convert_pix2world(idx_cloud, georef),
...
def generate_GIS_output(output: dict, settings: dict, inputs: dict):
    '''
    Encapsulate the generation of GEOJSON Layer from CoastSat.
    ...
    -----------
    output: dict
        Dictionary containing Metadata from filtered extracted shorelines
    inputs: dict
        contains predefined inputs from previous parameters
    Returns:
    -----------
    None

    '''
    st.toast('[PROCESS] Generating GIS Compatible Files.')
    geomtype = 'lines' # choose 'points' or 'lines' for the layer geometry [set to lines by default]
    gdf = tools.output_to_gdf(output, geomtype)
    if gdf is None:
    ...
    st.success('Shoreline Processing Complete!')
